...
<head>
    ...
    <title>Heroes - Editer un hero</title>
</head>
<body>
@extends('layout')
@section('content')
<form class="my-5 row" method="POST" action="{{ route('heroes.update', $heroe->id) }}" enctype="multipart/form-data">
     @csrf
    @method('PATCH')
    <h1 class="text-center col-12">Editer un hero</h1>
    ...
    <div class="mb-3 col-12">
        ...
        <input type="text" class="form-control" value="{{$heroe->name}}" name="name" id="name" aria-describedby="nameHelp">
    </div>
        <div class="mb-3 col-12">
        <label for="description" class="form-label">description du héro</label>
        <input type="text" class="form-control" value="{{$heroe->description}}" name="description" id="description" aria-describedby="descriptionHelp">
    </div>
    <div class="mb-3 col-6">
        ...
        <select class="form-select" name="skill" id="skill" aria-describedby="skillHelp">
            @foreach($skills as $skill)
                <option value="{{ $skill->id }}" {{  $skill->id == $heroe->skill ? "selected" : "" }} >{{ $skill->name }}</option>
            @endforeach
        </select>
    </div>
    <div class="mb-3 col-6">
        ...
        <select class="form-select" name="gender" id="gender" aria-describedby="genderHelp">
            @foreach($genders as $gender)
                <option value="{{ $gender->id }}" {{  $gender->id == $heroe->gender ? "selected" : "" }} >{{ $gender->name }}</option>
            @endforeach
        </select>
    </div>
    <div class="mb-3 col-6">
        ...
        <select class="form-select" name="race" id="race" aria-describedby="raceHelp">
            @foreach($races as $race)
                <option value="{{ $race->id }}" {{  $race->id == $heroe->race ? "selected" : "" }} >{{ $race->name }}</option>
            @endforeach
        </select>
    </div>
    <div class="mb-3 col-6">
    ...
    <div class="mb-3 col-12">
        ...
        <input type="file"  value="{{ $heroe->image }}" class="form-control" name="image" id="image" aria-describedby="imageHelp">
    </div>
